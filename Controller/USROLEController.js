...
const definePLSDBUSROLE = require('../Models/SDB/PLSDBUSROLE'); // Model factory
class USROLEController {
    constructor(dbName) {
        if (dbName) dbName = dbName == 'PLP00001SDB' ? 'A00001SDB' : dbName 
        this.connection = db.createPool(dbName);
        this.PLSDBUSROLE = definePLSDBUSROLE(this.connection);
    }
    async create(USRF01, USRF02, USRF03, USRF04, USRF05, USRF06, USRF07) {
        return await this.PLSDBUSROLE.create({
            USRF01, 
            USRF02, 
            USRF03, 
            USRF04, 
    ...
    async findAll(where = {}, orderby = [], attributes = null) {
        try {
            ...
            const results = await this.PLSDBUSROLE.findAll(options);
            return results;
        } catch (error) {
    ...
    async findOne(where = {}, orderby = [], attributes = null) {
        try {
            ...
            const result = await this.PLSDBUSROLE.findOne(options);
            return result;
        } catch (error) {
    ...
    async update(values, where = {}) {
        try {
            ...
            // Perform the update
            const [affectedCount, affectedRows] = await this.PLSDBUSROLE.update(values, {
                where,
    ...
    async destroy(where) {
        try {
            return await this.PLSDBUSROLE.destroy(where);
        } catch (error) {
...
module.exports = USROLEController