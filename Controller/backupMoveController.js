...
const backupToDrive = async (req, res) => {
    ...
    try {
        ...
        const databaseName = corporateID == 'PL-P-00001'
            ? `A${corporateID.slice(-5)}CMP${companyID.toString().padStart(4, "0")}`
            : `${CmpDBName.join('')}CMP${companyID.toString().padStart(4, "0")}`;
        ...
        await sequelizeMASTER.query(`
            BACKUP DATABASE [${databaseName}]
            TO DISK =  N'C:\\files\\${fileName}'
            WITH FORMAT, INIT, COPY_ONLY
        `);

        /* ========= DOWNLOAD BAK ========= */
        const fileUrl = `https://files.epluserp.cloud/${fileName}`;
        const bakBuffer = await downloadFileToBuffer(fileUrl);
        ...
        const zipBuffer = await createZipBuffer([
            { name: fileName, data: bakBuffer },
            { name: "metadata.json", data: jsonBuffer }
        ]);
        ...
        /* ========= EMAIL ========= */
            ...
            response.message = "Backup sent via email";
        }
...
const deleteFtpFile = async (req, res) => {
    ...
    try {
        ...
        console.log("🗑 Target FTP Path:", remotePath);

        /* ========= FTP CONNECT ========= */
        await ftpClient.access({
            host: process.env.FTP_HOST1,
            user: process.env.FTP_USER1,
            password: process.env.FTP_PASS1,
        ...
        /* ========= DELETE FILE ========= */
        try {
            await ftpClient.remove(fileName); // since we cd into /files
        } catch (ftpErr) {
        ...
        response.status = "FAIL";
        response.message = err.message || "Delete failed";
