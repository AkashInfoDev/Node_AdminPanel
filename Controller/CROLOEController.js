...
const definePLSDBCROLE = require('../Models/SDB/PLSDBCROLE'); // Model factory
class CROLEController {
    constructor(dbName) {
        if (dbName) dbName = dbName == 'PLP00001SDB' ? 'A00001SDB' : dbName 
        this.connection = db.createPool(dbName);
        this.PLSDBCROLE = definePLSDBCROLE(this.connection);
    }
    async create(CROLF01, CROLF02) {
        return await this.PLSDBCROLE.create({ 
            CROLF01, 
            CROLF02
        });
    }
    ...
    async findAll(where = {}, orderby = [], attributes = null) {
        try {
            ...
            const results = await this.PLSDBCROLE.findAll(options);
            return results;
        } catch (error) {
    ...
    async findOne(where = {}, orderby = [], attributes = null) {
        try {
            ...
            const result = await this.PLSDBCROLE.findOne(options);
            return result;
        } catch (error) {
    ...
    async update(values, where = {}) {
        try {
            ...
            // Perform the update
            const [affectedCount, affectedRows] = await this.PLSDBCROLE.update(values, {
                where,
